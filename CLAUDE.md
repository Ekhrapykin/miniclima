...
### Remote host
Target device is `ben` (Raspberry Pi 4B, `/home/khrap/miniclima`). Use `ssh ben` and `just push` to sync. Frontend available on `http://ben.local:3000/`. Deployment via docker. Use `just` command to build deployment code.